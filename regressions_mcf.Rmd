...
This study presents a comprehensive analysis of sentiment analysis methodologies applied to central bank statements, focusing on the Federal Open Market Committee (FOMC) announcements. We will start of in a regression manner similar to Möller and Reichmann's (2021) methodology. However, we will drift away from their approach of looking at macroeconomic factors, but will move towards looking at what market factors can drive abnormal returns. Through our investigation, several key conclusions emerge. Firstly, we observe a substantial improvement in explanatory capability when employing heuristics-adjusted dictionary approaches, such as VADER, particularly evident in more homogeneous texts. Secondly, we highlight the significance of incorporating the mechanical component of FOMC statements, specifically the interest rate announcements and associated surprises, to prevent biased estimates in central bank statement analyses. Additionally, we identify a subtle sensitivity of higher leverage periods to these surprises. Lastly, this research lays the groundwork for further exploration of the intersection between natural language processing and financial markets, particularly in event-study contexts.
...
ggplot(plotbands, aes(x = RegressionDates, y = Tone)) +
  ...
  labs(title = "Time Series with Error Bands",
       x = "Time", y = "Value")
...
## 6. Conclusion
Our analysis allows us to make a plethora of conclusions. First, 
